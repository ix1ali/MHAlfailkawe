...
import { BANKS, ContractSheet, EvictionSheet, PrintOverlay, ReceiptSheet, parcelNo } from "./print";
...
function ContractFormSheet({ onClose }: { onClose: () => void }) {
  ...
  const b = data.buildings.find((x) => x.id === activeBuilding) ?? data.buildings[0];
  const [open, setOpen] = useState(false);
  ...
  const [f, setF] = useState({
    ...
    area: b?.area ?? "",
    block: (b?.block ?? "").replace("قطعة", "").replace("رقم", "").trim(),
    street: (b?.street ?? "").replace("شارع", "").replace("رقم", "").trim(),
    buildingNo: parcelNo(b?.parcel),
      ...
      >
        <div className="space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <Field label="قسيمة رقم"><TextInput value={f.buildingNo} onChange={(e) => set("buildingNo", e.target.value)} /></Field>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="ينتهي بتاريخ"><TextInput type="date" value={f.endDate} onChange={(e) => set("endDate", e.target.value)} /></Field>
