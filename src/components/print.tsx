...
/** رقم القسيمة مجرّدًا: «قسيمة رقم 20/79» ← «20/79». */
export const parcelNo = (p?: string) => (p ?? "").replace("قسيمة", "").replace("رقم", "").trim();
  ...
  return (
    <div className="mb-5 flex items-start justify-between gap-4 border-b-2 pb-3" style={{ borderColor: NAVY }}>
      <div className="flex items-center gap-3">
        <Logo size={48} />
        <div>
          <p className="text-[17px] font-extrabold" style={{ color: INK }}>عقار / محمود الفيلكاوي</p>
          <p className="text-[11.5px]" style={{ color: MUTED }}>Real Estate / Mahmoud AlFailakawi</p>
...
/**
 * عقد الإيجار بنصّه وشكله المعتمد في المكتب — نسخة طبق الأصل من العقد الورقي:
 * حبر أسود، جدول الطرفين، عشرون بندًا، وتوقيعان. ورقة A4 واحدة دائمًا:
 * إن زاد النص (اسم طويل مثلًا) يُصغَّر الخط تلقائيًا حتى تتسع الصفحة.
 */
export function ContractSheet({ f }: { f: ContractFields }) {
  ...
  // ضبط الخط ليتسع العقد في صفحة واحدة
  useLayoutEffect(() => {
    const fit = () => {
      const page = pageRef.current, body = bodyRef.current;
      if (!page || !body) return;
      const room = page.clientHeight - parseFloat(getComputedStyle(page).paddingTop) - parseFloat(getComputedStyle(page).paddingBottom);
      let s = 12.5;
      body.style.fontSize = `${s}px`;
      while (body.scrollHeight > room && s > 8) {
        s -= 0.25;
        body.style.fontSize = `${s}px`;
      }
      setSize(s);
  ...
  }, [f, owner, data.settings.lateFee, data.settings.supervisorFee]);

  const K = "#000";
  const blank = (v: React.ReactNode, w = 90) => (
    <span
      className="inline-block border-b border-dotted text-center font-bold"
      style={{ minWidth: w, borderColor: K, padding: "0 4px" }}
    >
      {v || " "}
    </span>
  );
  const cell: React.CSSProperties = { border: `1px solid ${K}`, padding: "1px 6px", verticalAlign: "top" };

  const clauses: React.ReactNode[] = [
    <>
      مدة هذا العقد {blank(f.duration, 70)} يبدأ بتاريخ {blank(dateOrBlank(f.startDate))} وينتهي بتاريخ{" "}
      {blank(dateOrBlank(f.endDate))} وتتجدد تلقائيًا لمدد مماثلة ما لم يخطر أي من الطرفين الطرف الآخر بعدم رغبته في
      التجديد كتابةً قبل انتهاء المدة بشهر على الأقل، وعند حصول التنبيه بالإخلاء فإنه يجب على المستأجر أن يسهّل ويسمح
      بدخول كل من يرغب بمعاينة العين.
    </>,
    <>
      القيمة الإيجارية الشهرية {blank(f.rent ? amount(f.rent) : "", 90)} <b dir="ltr">K.D</b> تعتبر دينًا مترصدًا في ذمة
      الطرف الثاني محدد القيمة وواجب الوفاء.
    </>,
    ...
    <>
      في حالة رغبة المستأجر بالإخلاء وإنهاء العقد عليه إعلام المؤجر خطيًا قبل الإخلاء بمدة لا تقل عن شهر ويجب تسليم العين
      قبل تاريخ 25 من الشهر، وفي حال التأخير وعدم الالتزام بتسليم العين المؤجرة بالموعد فيُحتسب على الطرف الثاني إيجار
      الشهر الذي يليه.
    </>,
    <>
      يتعهد ويلتزم المستأجر بعدم الجلوس والتدخين بالمصاعد والممرات والسلالم، ولا يحق له التخزين في أي مكان خارج العين
      المؤجرة أو استخدامها كمخزن للأغراض أو المواد الملتهبة أو المضرة بالصحة، وهو مسؤول قِبل المؤجر عن أي حريق يحدث نتيجة
      إهماله أو تعديه، ولا يحق للمستأجر اعتبار المؤجر مسؤولًا عن تعدي الغير، وفي حالة المخالفة يحق للطرف الأول رمي أي
      أغراض خُزّنت خارج العين ولا يحق له المطالبة بتعويض، كما يلتزم بعدم رمي الأوساخ خارج المكان المخصص لذلك وعدم استخدام
      مواقف السيارات بطريقة خاطئة.
    </>,
    <>
      إذا تأخر الطرف الثاني عن دفع القيمة الإيجارية في ميعاد استحقاقها يُفسخ العقد فورًا من تلقاء نفسه دون الحاجة إلى تنبيه
      أو إنذار، وتعتبر يد المستأجر يد غاصب، ويختص القضاء المستعجل بالحكم بصفة مستعجلة بطرده من العين، وكذلك من حق المؤجر
      المطالبة بكامل قيمة العقد عن مدته الأصلية أو المحددة والتعويض الاتفاقي في البند الأول واستيفاء المستأجر لكافة
      التعويضات المصاريف المترتبة على الإخلاء.
    </>,
    <>
      يقر الطرف الثاني أنه عاين العين محل العقد المعاينة التامة النافية للجهالة وقد وجدها على أحسن حال ومستوفية لكل لوازمها
      التي تمكنه من الانتفاع بها.
    </>,
    <>
      يتعهد الطرف الثاني بالمحافظة على العين المؤجرة وتسليمها على حالتها كما استلمها، وألّا يُحدث أي تغيير سواء هدم أو بناء
      أو تمديدات كهربائية والستالايت إلا بتصريح كتابي من الطرف الأول.
    </>,
    <>
      مصروفات رسوم الكهرباء والماء وبلدية الكويت يتحملها الطرف الأول، كما يتحمل الطرف الثاني دفع أي زيادة أو إضافة في تسعيرة
      الخدمات التي تقدمها الجهات الحكومية أو الأهلية كزيادة رسوم مصروفات الماء والكهرباء والنظافة وضريبة القيمة المضافة
      وغيرها من الرسوم التي قد تُفرض على العين المؤجرة، وتعتبر هذه الزيادة جزءًا من هذا العقد الماثل وتضاف على القيمة
      الإيجارية المحددة والملزم بدفعها الطرف الثاني.
    </>,
    <>جميع المنقولات الموجودة بالعين المستأجرة ضامنة للأجرة المعقود عليها، ولا يحق للمستأجر نقلها إلا بعد الوفاء بالأجرة.</>,
    <>
      لا يحق للطرف الثاني التأجير بالباطن أو إيواء الغير أو التنازل عن المكان أو جزء منه دون أخذ موافقة خطية من المؤجر، مع
      دفع قيمة الإيجار مقدمًا في حالة السفر، كما أن الموكلين من الطرف الثاني بإدارة العين أو القاطنين يعتبرون ضامنين
      متضامنين بدفع جميع المبالغ المستحقة من الإيجار، ولا يحق لهم تغيير حق السكن العائلي وذلك يعتبر مخالفة لبنود العقد.
    </>,
    <>
      يتعهد ويلتزم المستأجر بعدم الإضرار بالجيران وإزعاجهم، وأن يحترم الشعائر والأحكام الإسلامية والعادات والتقاليد والأعراف
      في دولة الكويت، ويكون مسؤولًا عن تصرفاته الشخصية وعن تصرفات التابعين له ومن يكون قاطنًا معه، ويُمنع منعًا باتًا إدخال
      أي نوع من الحيوانات داخل العقار.
    </>,
    <>
      في حال عدم التزام الطرف الثاني بسداد الأجرة الشهرية، يلتزم الطرف الثاني بدفع مبلغ {amount(data.settings.lateFee)} دينار
      كويتي للطرف الأول وذلك نظير الرسوم الإدارية وأتعاب المحاماة.
    </>,
    <>
      يتعهد ويلتزم الطرف الثاني بجميع ما ذُكر بالعقد حتى في حال تعطيل الدوامات في القطاع الحكومي أو الأهلي أو نشوب كوارث
      طبيعية أو حروب أو ظروف خارجة عن الإرادة أو حالات صحية أو الانقطاع أو التضرر الكلي أو الجزئي في وظيفة وعمل الطرف الثاني.
    </>,
    <>
      في حال مخالفة الطرف الثاني لأي بند من البنود المذكورة في هذا الاتفاق، يعتبر العقد مفسوخًا من تلقاء نفسه دون الرجوع
      إليه، ويحق للطرف الأول المطالبة بتعويض عن الأضرار التي تنتج عن ذلك.
    </>,
    <>
      يقر المستأجر بأنه يتخذ العين موضوع العقد محلًا مختارًا له المبين عنوانها، وكذلك ( البريد الإلكتروني ) ورقم الهاتف
      المسجل في صدر هذا العقد، وكل إعلان يُرسل له عن طريق البريد الإلكتروني ورقم الهاتف أو عنوان السكن أو أي وسيلة تقرها
      دولة الكويت يعتبر قانونيًا.
    </>,
    <>
      يدفع المستأجر مبلغ {amount(data.settings.supervisorFee)} دنانير كويتي للمشرف الفني على العمارة مع دفع الأجرة الشهرية
      وذلك نظير قيامه بتنظيف العمارة ورمي القمامة.
    </>,
    <>
      كل ما لم يرد به اتفاق في هذا العقد يخضع لقانون الإيجارات بدولة الكويت، وتختص المحاكم الكويتية بالفصل في المنازعات
      الناشئة عن تنفيذ هذا العقد.
    </>,
  ...
  return (
    <div className="overflow-x-auto print:overflow-visible">
      <div
        ...
        dir="rtl"
        style={{
          width: "210mm", height: "296mm", padding: "12mm 13mm 10mm", boxSizing: "border-box",
          overflow: "hidden", background: "#fff", color: K, margin: "0 auto",
          fontFamily: '"Times New Roman", "Simplified Arabic", "Traditional Arabic", Arial, serif',
        }}
      >
        <div ref={bodyRef} style={{ fontSize: size, lineHeight: 1.45 }}>
          <div className="text-center" style={{ marginBottom: "0.9em" }}>
            <p className="font-bold underline" style={{ fontSize: "1.25em" }}>عقد ايجار</p>
            <p className="font-bold underline" dir="ltr" style={{ fontSize: "1.25em" }}>Rent Contract</p>
          </div>

          <div className="flex items-baseline justify-between gap-4 font-bold" style={{ marginBottom: "0.3em" }}>
            <span>
              في الكويت اليوم : {blank(dayName(f.startDate), 70)} &nbsp; الموافق : {blank(dateOrBlank(f.startDate), 90)}
            </span>
            <span>تحرر وتم الاتفاق بين كل من الطرفين</span>
          </div>

          <table className="w-full border-collapse font-bold" style={{ marginBottom: "0.5em", tableLayout: "fixed" }}>
            <tbody>
              <tr>
                <td style={cell}>الطرف الأول ( المؤجر ) :</td>
                <td style={cell}>الطرف الثاني ( المستأجر ) : {f.tenantName}</td>
              </tr>
              <tr>
                <td style={cell} rowSpan={4}>السيد / {owner}</td>
                <td style={cell}>الرقم المدني : {f.civilId}</td>
              </tr>
              <tr><td style={cell}>الجنسية : {f.nationality}</td></tr>
              <tr><td style={cell}>المهنة : {f.job}</td></tr>
              <tr><td style={cell}>رقم الهاتف : <span dir="ltr">{f.phone}</span></td></tr>
            </tbody>
          </table>

          <p className="font-bold" style={{ marginBottom: "0.3em", textAlign: "justify" }}>
            على ان يؤجر الطرف الأول للطرف الثاني ( العين ) الواقعة بمنطقة ( {f.area} ) قطعة رقم ( {f.block} ) شارع ({" "}
            {f.street} ) قسيمة رقم ( {f.buildingNo} ) الدور {blank(f.floor, 60)} شقة رقم {blank(f.unitNo, 50)} كسكن خاص له
            ولعائلته وفق الشروط التالية:
          </p>

          <ol style={{ margin: 0, padding: 0, listStyle: "none" }}>
            {clauses.map((c, i) => (
              <li key={i} className="flex gap-1.5" style={{ textAlign: "justify", marginBottom: "0.1em" }}>
                <span className="shrink-0 font-bold" style={{ width: "1.6em" }}>{i + 1} .</span>
                <span className="flex-1">{c}</span>
              </li>
            ))}
          </ol>

          <div className="grid grid-cols-2 text-center font-bold" style={{ marginTop: "1.1em" }}>
            <div>
              <p>الطرف الأول</p>
              <p>( المؤجر )</p>
              <p style={{ marginTop: "0.4em" }}>{owner}</p>
            </div>
            <div>
              <p>الطرف الثاني</p>
              <p>( المستأجر )</p>
              <p style={{ marginTop: "1.2em", letterSpacing: 2 }}>..............................................</p>
              <p style={{ marginTop: "1.2em", letterSpacing: 2 }}>..............................................</p>
            </div>
          </div>
        </div>
  ...
  return (
    <ContractSheet
      f={{
        ...
        phone: tenant?.phone ?? "",
        area: building?.area ?? "",
        block: (building?.block ?? "").replace("قطعة", "").replace("رقم", "").trim(),
        street: (building?.street ?? "").replace("شارع", "").replace("رقم", "").trim(),
        buildingNo: parcelNo(building?.parcel),
        floor: floor?.name ?? "",
        unitNo: unit?.number ?? "",
    ...
    <div style={{ color: INK }}>
      ...
      <div className="grid grid-cols-3 items-start gap-3" style={{ marginBottom: compact ? 16 : 18 }}>
        ...
        <div className="text-left" dir="ltr">
          <p style={{ fontSize: S.org - 1.5, fontWeight: 800, lineHeight: 1.3 }}>Mahmoud AlFailakawi</p>
